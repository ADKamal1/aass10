...
import 'package:basket/_model/orderSummary.dart';
...
// get all orderss for the custoemrs..
  Future<List<orderSammury>> getAllOrdersForCustomer() async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM orderss');
    List<orderSammury> customerOrders = new List();
    for (int i = 0; i < list.length; i++) {
      orderSammury order =
      new orderSammury(orderId: list[i]["orderId"],
          regionId: list[i]["regionID"],
          orderDate: list[i]["orderDate"],
          totalSalary: list[i]["totalSalary"]);
...
//
//  Future<List<orderSammury>> getMyOrders() async {
//    var dbClient = await db;
//    List<orderSammury> list = await dbClient
//        .rawQuery('SELECT * FROM order');
//    // print("here  = " + list.elementAt(0)["quantity"].toString());
...
//  }
 ...
 // print("here  = " + list.elementAt(0)["quantity"].toString());
  ...
  Future<List<Favourite>> getAllBasketProductPaths() async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM basket');
    List<Favourite> productPaths = new List();
    for (int i = 0; i < list.length; i++) {
      var favourite =
          new Favourite(list[i]["productPath"], list[i]["category"] , list[i]["quantity"]);

      productPaths.add(favourite);
    }

    return productPaths;
  }

  Future<List<Favourite>> getAllProductPaths() async {
    var dbClient = await db;
    List<Map> list = await dbClient.rawQuery('SELECT * FROM favourite');
    List<Favourite> productPaths = new List();
    for (int i = 0; i < list.length; i++) {
      var favourite =
          new Favourite(list[i]["productPath"], list[i]["category"] , 0);
